...
class BriefTests(unittest.TestCase):
    def test_generate_brief_uses_live_market_frames_without_external_ai(self):
        ...
        self.assertIn("<b>Готуватись до LONG — ранній від підтримки:</b>", text)
        self.assertIn("зона $95.00-$97.00 тримається", text)
        self.assertIn("цілі: $115.00 → $119.00 → $123.00", text)
        self.assertIn("<b>Готуватись до LONG — консервативний:</b>", text)
        self.assertIn("1h close &gt; $115.00 + ретест зверху", text)
        self.assertIn("<b>Готуватись до SHORT — ранній від опору:</b>", text)
        self.assertIn("цілі: $95.00 → $91.00 → $87.00", text)
        self.assertIn("План: ранній LONG тільки від підтримки після повернення вище зони", text)
        self.assertIn("<b>Окремий LONG/SHORT alert має сенс, коли:</b>", text)
        self.assertIn("ранній сценарій від підтримки/опору активувався або є пробій + ретест", text)
